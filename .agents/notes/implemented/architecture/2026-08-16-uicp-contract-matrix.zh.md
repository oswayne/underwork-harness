...
[`@deepseek-ai/dsh-contract-matrix`](../../../../packages/uicp/contract-matrix/README.zh.md) 提供固定语料（全部 18 个操作符、回退/跳过语义、排序、分页、`_preventListAll`、stats、日期/布尔转换、带唯一与构造函数生命周期的 CRUD、静态/对象函数路径、树形查询与分支）以及 `runMatrix` / `diffMatrix`。`buildReferenceTarget()` 用固定 `_id` 种子构建确定性进程内沙盒作为本地基准；平台适配器在基准环境就绪时接入。
