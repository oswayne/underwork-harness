...
- 多用户模型（单实例服务端部署）：预览 seam 的所有路由要求 `Authorization: Bearer <JWT>`（401 拒绝匿名）；沙盒按 JWT 派生的用户键隔离，同一应用包目录每个凭据一份独立的内存数据。同一应用包文件的并发保存为"最后写入者胜"，多人同时编辑需人工协调（后续可选加乐观并发/版本冲突提示）。
- 部署注意事项：远程部署必须走 HTTPS（JWT 经 URL/请求头发送，明文 HTTP 会泄露凭据）；同一浏览器配置内仍是单用户（localStorage 单 key），多用户请使用各自独立的浏览器环境。
...
- [x] 应用包目录结构与文件格式按第 3 章落地；示例应用包（1 租户 + 1 App + 1 Entity + 2 Function + 1 列表页 + fixture）。
- [x] 人工导入平台验证字段映射与 API 可行（2026-08-16，dsh-test 平台侧记录核验通过）。

### M2：知识注入 + 预览

- [x] preset + skill 上线，agent 能按契约生成示例包。
- [x] `apppackage_validate` 静态校验矩阵生效（sre-w 仅存 audit-create 页面差异，已移交 eureka 团队）。
- [x] eureka 渲染器内嵌预览（fixture/沙盒数据）可渲染页面；上传组件 mock 可用（沙盒数据路径已接入，见 M3）。

### M3：完整闭环

- [x] 沙盒：CRUD/查询（含 stats/树形）/函数执行/构造函数/上传 mock 全部可交互；行为矩阵契约测试跑通（平台对比 37/45，8 项差异为平台侧缺陷）。
- [x] eureka-editor 干预写回本地并重新校验。
- [x] 自动测试（正反例/边界）覆盖矩阵生效、失败回喂修复。
- [x] 版本快照/切换/回滚可用。
- [x] API 保存（逐记录 upsert、幂等、测试数据不写平台）跑通"需求 → 生成 → 校验 → 预览/干预 → 自动测试 → 采纳 → 保存"。
