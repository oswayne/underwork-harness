...
桌面驱动器的 UICP 平台导航插件：JWT 登录、侧栏的"租户 → 应用包 → 会话"浏览、以及会话头的会话切换。
...
平台 Token 经壳的 `get_token` / `set_token` / `clear_token` 命令存取（钥匙串在后续里程碑接入），无 Tauri 桥时以内存兜底。Token 永不进入 localStorage/sessionStorage。
