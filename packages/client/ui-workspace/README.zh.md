...
可选的 `managedWorkspaces` 上下文守卫（声明于 `src/client/contract/managed.ts`）让宿主组合标记用户不得删除的 workspace；浏览器对它们隐藏删除入口。uicp 组合通过 `ui-uicp-nav` 提供该守卫；服务缺失时所有 workspace 都可删除。
