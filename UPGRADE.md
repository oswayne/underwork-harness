...
> **方案修订（2026-08-19）**：放弃桌面壳（Tauri）方向，产品形态改为 **B/S**——以 `dsh web` 本地 HTTP 服务为产品本体，浏览器为 UI 载体（localStorage 持久 Token）。原 D7/D20/D21/D22 及 3.9/3.16–3.18 的壳相关决策整体废止；D19 保留（本来就是 localhost HTTP 三合一）。UICP 组装从上游 `dsh-web-app` bundle 抽出为独立 `@deepseek-ai/dsh-uicp-web-app` bundle，避免上游合并冲突。
...
目标：把 dsh 改造成平台的"生成驱动器"，并以 **B/S 形态**交付（`dsh web` 本地服务 + 浏览器 UI）。dsh 通过**手动输入 JWT 为主（Passkey 为可选增强）**完成认证，最终得到平台 Token（JWT）作为所有平台 API 请求的必须参数；**每个可用租户即一个项目**，无独立项目概念；租户下按**应用包（App）**组织，用户选择应用包后新建或继续对话；此后 dsh 在该租户、该应用包内设计并生成完整产物——Entity 定义、与 Entity 关联的 Function、页面 JSON（含菜单挂载），交付平台运行；开发完成后由 dsh 自动测试（**覆盖正反例与边界，不只快乐路径**），在**数据交互与 uicp-server 一致的本地沙盒**（数据保存、查询、函数执行）中完成全部验证与测试，用户采纳后经 API 保存完成发布（**API 保存是最后一步**，发布后由人工在平台侧走快乐路径兜底）；支持**从零搭建应用包**：Entity 建模（字段/关联）与测试数据生成，用于页面/函数开发与预览；页面 JSON 可在浏览器内嵌的 eureka 渲染器中预览，并可用 eureka-editor 局部干预（独立浏览器窗口）；后续修改需求时，dsh 增量修改对应产物，且全程不触碰其他租户的数据。

### 1.1 设计原则（用户指定，全局约束）

- **B/S 服务于 dsh**：本地 HTTP 服务是 dsh 的载体，不是独立产品。界面沿用 dsh Web UI 原生风格（基于 dsh Web UI + 最小改造，D14/D18 修订），不引入独立视觉体系；dsh 功能（应用包工作区、生成、校验、沙盒、预览/编辑器、自动测试、版本管理）始终是第一优先级；任何改造不得牺牲功能可达性与既有扩展点。
...
- Token 存于浏览器 localStorage（不落代码、不进会话日志、不进产物文件），每次平台请求附加 `Authorization` 与当前项目（租户）的 `Tenant` 头；**持久化（D5 已定）**：启动时自动读取并校验，失效才要求重新输入/登录。
- 平台 API 由浏览器 Web UI 直接调用（平台已支持浏览器跨域，web-admin 同模型），Token 由 localStorage 注入请求头。
...
- 主题：沿用 dsh 现有主题（3.9）。
- 认证与凭证：JWT 输入 + localStorage 持久化；Passkey（可选增强）走浏览器原生 WebAuthn（3.16）。
...
### 3.9 B/S 应用形态（2026-08-19 修订：废止桌面壳）

- dsh 以 B/S 交付：`dsh web`（本地 HTTP 服务三合一：dsh Web UI 页面 + RPC/事件流 + 沙盒 API）即产品本体，浏览器直接访问 `http://127.0.0.1:<port>`（默认 3080）。
- 端口固定（默认 3080）是 B/S 的持久化前提：localStorage 按 origin（host:port）隔离，固定端口才能让 Token 跨重启持久（原桌面壳用 `--port 0` + 文件镜像，已随壳废止）。
- 认证为 JWT 输入为主、Passkey 可选增强（3.2）：浏览器原生 WebAuthn 可用时不依赖壳桥，仍是可选增强。
- Token 与租户绑定存浏览器 localStorage（SharedPreferences 语义），替代桌面安全存储；会话恢复、失效重输流程与 web-admin 一致。
...
- B/S 附带工作：启动/停止脚本（或服务包装）、固定端口约定、浏览器打开；无打包、签名、托盘。
...
- 集成契约：按 eureka 官方示例（examples、docs/start/getting-started）组装渲染器/编辑器；构建验收在 M2；编辑器在独立浏览器窗口（`window.open` → `/uicp/editor`）承载，与主对话页隔离（2026-08-19 修订）。
...
- **目标**：复用 dsh Web UI，用最小改造承载全部功能；体积与复杂度通过端点裁剪与懒加载控制（"最小 host 只留核心"让位于 UI 复用，是 D18 修订的固有代价）。
...
- **轻量化**：monaco 懒加载、非必需端点裁剪；上游同步范围 = 核心 + Web UI（跟随），冲突面控制在适配层与补丁清单（第 5 节）。

### 3.16–3.18 壳桥面 / 进程生命周期 / 打包分发（已废止，2026-08-19）

随桌面壳方向整体废止（D20/D21/D22）。B/S 形态下：
- 凭证存储由浏览器 localStorage 承担（见 3.9），无壳桥。
- 进程生命周期由启动命令/服务管理（`dsh web` 前台或后台启动），无 sidecar spawn、无单实例、无托盘。
- 无应用打包、签名、公证、更新通道；上游同步后直接重新 `pnpm install && pnpm run build` 即可使用。

## 4. 实施阶段

- M0 B/S 形态 + UI 集成原型：`dsh web` + 浏览器加载，打通"JWT 输入 → 校验 → 列租户 → 进入租户 → 选择/新建应用包 → 开始对话"（Passkey 为可选增强，如启用则一并验收）；验收清单：
  - localhost HTTP 三合一加载（D19）与 dsh Web UI 集成（client 插件 / 适配层，D18 修订）；
  - JWT 输入为主路径；Passkey（可选增强，浏览器原生 WebAuthn）如启用则验收；
  - 进程基础：`dsh web` 启动 / 停止（D21）；
  - 交付：可运行的 B/S 应用（功能闭环由 M1–M3 完成）。
...
- **新增优先于修改**：新代码放独立包/目录（如 `packages/uicp/*`、`packages/client/ui-uicp-*`、`apps/cli/config/agent-presets/` 新增目录）；避免编辑上游已有文件。UICP 的 Web 组装层是独立 bundle `@deepseek-ai/dsh-uicp-web-app`（`packages/bundle/uicp-web-app/`），在 profile 模板中列于 `dsh-web-app` 之后——上游 `web-app/cordis.patch.yml` 保持零改动，上游更新该文件不产生冲突。确需修改的共享文件集中在固定清单（pnpm-workspace.yaml、tsconfig.host.json / tsconfig.client.json、根 package.json、docs 导航、`packages/boot/app-boot/src/profile.ts` 的 profile 模板一行），改动保持最小并记录在案。
...
- D3 浏览器内嵌 eureka 渲染器预览 + eureka-editor 局部干预（独立浏览器窗口），无头渲染仅作 CI 补充（见 3.7）。
- D4 知识注入深度：preset 承载行为规则 + skill 承载契约词汇（按需查询），校验工具兜底（见 3.3）。
- D5 Token 持久化：启动自动恢复（localStorage） + 失效重输（见 3.2）。
- D6 认证方式：手动输入 JWT 为主路径，Passkey 为可选增强（平台接口与约束已核实），两者最终产出平台 Token（JWT）作为所有平台 API 请求的必须参数（见 2.5 / 3.2）。
- D7 桌面壳已废止（2026-08-19）：产品形态改为 B/S，`dsh web` + 浏览器；不再评估 Tauri/Electron。
...
- D20 壳原生桥面已废止（2026-08-19）：凭证存储改浏览器 localStorage；Passkey 走浏览器原生 WebAuthn（可选增强）；无托盘/通知/外链桥。
- D21 进程生命周期已废止（2026-08-19）：由启动命令/服务管理，无 sidecar spawn/单实例/托盘。
- D22 打包与分发已废止（2026-08-19）：B/S 无应用打包、签名、更新通道；上游同步后重新构建即可。
...
| 方案能力 | dsh 机制（代码位置） | 结论 |
| --- | --- | --- |
| B/S 服务承载 Web UI | `apps/web`（vite 构建）+ `apps/cli` `dsh web`；`packages/bundle/web-app` 默认端口 3080（固定，localStorage 持久化前提） | 可行 |
...
| Token 存储 | 浏览器 localStorage（`uicp.platform.token`）+ 启动时 `/user/user/self` 校验（见 3.2） | 可行 |
